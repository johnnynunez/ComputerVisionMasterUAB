...
def run_mot(cfg: CfgNode):
    ...
    for frame_num, frame in enumerate(video_in):
        ...
        res = detector(frame)[0].boxes.cpu().numpy()
        ...
        for det in res:
            ...
            boxes.append([det.data[0][0],det.data[0][1],w,h]) """
        
        boxes = [t for t in res.xywh]
        scores = [t for t in res.conf]
        classes = [t for t in res.cls]
