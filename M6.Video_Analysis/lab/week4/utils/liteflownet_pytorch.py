...
# ======== PLEASE MODIFY ========
# where is the repo
repoRoot = '/ghome/group03/mcv-m6-2023-team6/week4/pytorch-liteflownet'
# to CUDA\vX.Y\bin
...
class Network(torch.nn.Module):
    def __init__(self):
        ...
        class Regularization(torch.nn.Module):
            ...
            def forward(self, tenOne, tenTwo, tenFeaturesOne, tenFeaturesTwo, tenFlow):
                tenDifference = (tenOne - backwarp(tenInput=tenTwo, tenFlow=tenFlow * self.fltBackward)).square().sum(
                    [1], True).sqrt().detach()

                tenDist = self.netDist(self.netMain(
                    torch.cat([tenDifference, tenFlow - tenFlow.mean([2, 3], True), self.netFeat(tenFeaturesOne)], 1)))
...
def flow_liteflownet(image1, image2, colType=1, model='kitti'):
    tenOne = torch.FloatTensor(numpy.ascontiguousarray(
        numpy.array(image1)[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))
    tenTwo = torch.FloatTensor(numpy.ascontiguousarray(
        numpy.array(image2)[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))
    ...
    return tenOutput.numpy().transpose(1, 2, 0), end - start
