...
if __name__ == '__main__':
    ...
    for d in random.sample(dataset_dicts, 20):
        img_2_d = random.sample(dataset_dicts, 1)[0]
        img_1 = cv2.imread(d["file_name"])
        img_2 = cv2.imread(img_2_d["file_name"])
        ...
        mask = outputs_img1["instances"].pred_masks[0].numpy()
        mask = np.repeat(mask[:, :, np.newaxis], 3, axis=2)
        img_1 = img_1 * mask + (1 - mask) * 255

        # IMATGE 2 ORIGINAL
        cv2.imwrite(output_path + "pred_" + d["file_name"].split('/')[-1], img_1)
